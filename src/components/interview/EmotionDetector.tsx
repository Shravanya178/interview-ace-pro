...
// Emotion labels
const EMOTIONS = ['confident', 'nervous', 'engaged', 'disinterested', 'thoughtful', 'uncertain', 'neutral'];
...
const EmotionDetector: React.FC<EmotionDetectorProps> = ({
  ...
  const generateEmotions = useCallback(() => {
    ...
    try {
          ...
          for (let y = 0; y < imageData.height; y++) {
            ...
            for (let x = 0; x < imageData.width; x++) {
              ...
              if (y > 0 && x < imageData.width - 1) {
                ...
                const aboveBrightness = (imageData.data[aboveIdx] + imageData.data[aboveIdx+1] + imageData.data[aboveIdx+2]) / 3;
          ...
          if (pixelMatrix.length > 2) {
            ...
            for (let y = 1; y < pixelMatrix.length - 1; y++) {
              for (let x = 1; x < pixelMatrix[y].length - 1; x++) {
                ...
                const neighbors = [
                  pixelMatrix[y-1][x-1], pixelMatrix[y-1][x], pixelMatrix[y-1][x+1],
                  pixelMatrix[y][x-1], pixelMatrix[y][x+1],
                  pixelMatrix[y+1][x-1], pixelMatrix[y+1][x], pixelMatrix[y+1][x+1]
                ];
      ...
      const prevFrameData = emotionHistory[emotionHistory.length - 1]?.frameData;
      const prevRegionMotion = emotionHistory[emotionHistory.length - 1]?.regionMotion || {};
      
      if (prevFrameData && prevFrameData.data.length === currentFrame.data.length) {
        ...
        regions.forEach(region => {
          try {
            ...
            const regionMotionLevel = regionPixelCount > 0 ? regionDiffCount / regionPixelCount : 0;
            regionMotion[region.name] = regionMotionLevel;
            
            // Update specific region motion variables
            if (region.name === 'eyes' || region.name === 'leftEye' || region.name === 'rightEye') {
            ...
            console.warn(`Error calculating motion for region ${region.name}:`, err);
          }
        });
      }
      ...
      if (faceData && faceData.region === 'face' && 
          eyesData && eyesData.avgBrightness > 100) {
      ...
      if (motionLevel < 0.03 && emotionHistory.length > 0) {
        ...
        const lowMotionHistory = emotionHistory.slice(-5).filter(h => 
          h.motionLevel && h.motionLevel < 0.04
        );
      ...
      if (eyesData && eyesData.avgBrightness < 95 && faceData && faceData.avgBrightness > 100) {
      ...
      if (faceData && faceData.horizontalSymmetry < 0.7 && leftEyeData && rightEyeData) {
        const eyeHeightDiff = Math.abs(leftEyeData.avgBrightness - rightEyeData.avgBrightness);
        if (eyeHeightDiff > 10) {
      ...
      if (motionLevel < 0.08 && eyeMotion < 0.1 && faceData.avgBrightness > 110) {
        emotions.engaged += 0.3;
        emotions.neutral += 0.1;
      }
      
      // Detect "speaking with confidence" behavior
      if (mouthMotion > 0.12 && motionLevel < 0.15 && faceData.horizontalSymmetry > 0.75) {
        emotions.confident += 0.5;
        emotions.nervous -= 0.2;
      }
      
      // Detect "thinking before answering" behavior
      if ((eyeMotion > 0.1 || foreheadData?.horizontalEdgeRatio > 0.06) && 
          mouthMotion < 0.05 && motionLevel < 0.1) {
        emotions.thoughtful += 0.5;
        emotions.engaged += 0.2;
      }
      ...
      const consecutiveLowBrightness = emotionHistory.slice(-4).every(h => 
        h.faceData && h.faceData.avgBrightness < 95
      );
      ...
      if (motionLevel > 0.08) {  // Reduced threshold
      ...
      if (emotionHistory.length > 0) {
        ...
        Object.keys(emotions).forEach(emotion => {
          if (prevEmotions[emotion]) {
            emotions[emotion] = emotions[emotion] * (1 - smoothingFactor) + 
  ...
  }, [isActive, onEmotionCapture, cameraActive, videoRef, width, height, emotionHistory, currentEmotion, generateRandomEmotions]);
    ...
    <Card className="mb-4 overflow-hidden">
      <CardHeader className="pb-2">
        <CardTitle className="text-lg flex justify-between items-center">
          ...
          {currentEmotion && (
            <span className={`text-sm px-2 py-1 rounded-full text-white ${EMOTION_COLORS[currentEmotion.label as keyof typeof EMOTION_COLORS] || 'bg-gray-500'}`}>
      ...
      <CardContent className="p-3">
        {isLoading ? (
          ...
          <div className="flex flex-col">
            <div className="relative bg-black rounded-md" style={{ minHeight: '240px' }}>
              {/* Video element */}
              <video
              ...
              {currentEmotion && (
                <div 
                  ...
                  style={{ 
                    backgroundColor: currentEmotion.label === 'confident' ? 'rgba(16, 185, 129, 0.9)' :
                                   currentEmotion.label === 'nervous' ? 'rgba(236, 72, 153, 0.9)' :
                                   currentEmotion.label === 'engaged' ? 'rgba(59, 130, 246, 0.9)' :
                                   currentEmotion.label === 'disinterested' ? 'rgba(156, 163, 175, 0.9)' :
                                   currentEmotion.label === 'thoughtful' ? 'rgba(168, 85, 247, 0.9)' :
                                   currentEmotion.label === 'uncertain' ? 'rgba(245, 158, 11, 0.9)' : 
                                   'rgba(100, 116, 139, 0.9)',
                    transform: 'scale(1.05)',
                    boxShadow: '0 0 10px rgba(0,0,0,0.3)'
                  }}
                >
                  {currentEmotion.label} {(currentEmotion.confidence * 100).toFixed(0)}%
                </div>
              )}
              ...
              <div className="absolute bottom-2 left-2 right-2 flex flex-wrap gap-1">
                {EMOTIONS.map(emotion => {
                  const currentValue = emotionHistory.length > 0 ? 
                    emotionHistory[emotionHistory.length - 1].emotions[emotion] || 0 : 0;
                  ...
                  return (
                    <div 
                      ...
                      style={{ 
                        backgroundColor: emotion === 'confident' ? 'rgba(16, 185, 129, 0.9)' :
                                       emotion === 'nervous' ? 'rgba(236, 72, 153, 0.9)' :
                                       emotion === 'engaged' ? 'rgba(59, 130, 246, 0.9)' :
                                       emotion === 'disinterested' ? 'rgba(156, 163, 175, 0.9)' :
                                       emotion === 'thoughtful' ? 'rgba(168, 85, 247, 0.9)' :
                                       emotion === 'uncertain' ? 'rgba(245, 158, 11, 0.9)' : 
                                       'rgba(100, 116, 139, 0.9)',
            ...
            <div className="mt-4">
              <h4 className="text-sm font-medium mb-2">Interview Demeanor Metrics</h4>
              <div className="space-y-2">
                {EMOTIONS.map(emotion => {
                  const value = averageEmotions[emotion] || 0;
                  const instantValue = emotionHistory.length > 0 ? 
                    emotionHistory[emotionHistory.length - 1].emotions[emotion] || 0 : 0;
                  ...
                  return (
                    <div key={emotion} className="flex items-center">
                      ...
                      <div className="flex-1 h-5 bg-gray-100 rounded-full overflow-hidden relative">
                        ...
                        <div 
                          className={`h-full ${EMOTION_COLORS[emotion as keyof typeof EMOTION_COLORS]}`} 
                          style={{ 
                        ...
                        <div 
                          ...
                          style={{ 
                            left: `calc(${(instantValue * 100).toFixed(0)}% - 1px)`,
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-md">
              <p className="text-xs text-blue-800">
                ...
                Maintain steady eye contact for confidence, 
                control micro-movements to reduce nervousness, 
                lean slightly forward to show engagement, 
                nod occasionally to demonstrate active listening,
                and take thoughtful pauses before answering difficult questions.
              </p>
            </div>
          </div>
        )}
